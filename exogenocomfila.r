...
modificarTimelinesFIFO <- function(e){
    ...
    gerarEventosEndogenos(e[4])

    if(e[1] == e[4]) {
	    gerarEventosExogenos(e[4], e[2])
    }
}
...
gerarEventosExogenos <- function(quem, isFake){
    if(!isFake & L0 > 0){
        evento <- c(quem, GOOD, T+rexp(1,L0), quem)
        FilaDeEventos[[length(FilaDeEventos)+1]] <<- evento   
    }
    else if (isFake & L1 > 0){
        evento <- c(quem, FAKE, T+rexp(1,L1), quem)
        FilaDeEventos[[length(FilaDeEventos)+1]] <<- evento   
    }
}
...
gerarEventosEndogenos <- function(quem){
    ...
    ###retirarEventosAnteriores
    s <- length(FilaDeEventos)
    if (s > 0) {
        for (i in s:1) {
            evento <- FilaDeEventos[[i]]
            if(evento[1] == quem & evento[4] != quem){
                FilaDeEventos[i] <<- NULL
            }
        }
    }
